...
def write_custom_rule(rule, **kwargs):
    print(f"Writing custom rule for {rule.rule_id} to include tag {kwargs['custom_tags']}")
    ...
    if os.path.exists(f"../custom/rules/{rule.rule_id}.yaml"):
        ...
        rule_yaml = {}
    # add reference placeholder to rule_yaml
    if kwargs['custom_refs'] == "y":
        rule_yaml.setdefault("references", {})
        rule_yaml['references'].setdefault(kwargs['custom_tags'],[])

    # add tag to rule_yaml
    rule_yaml.setdefault("tags", []).append(kwargs['custom_tags'])
    with open(f"../custom/rules/{rule.rule_id}.yaml", 'w') as f:
      yaml.dump(rule_yaml, f, explicit_start=True)
...
def odv_query(rules, benchmark, **kwargs):
    ...
    for rule in rules:
        ...
        if include.upper() == "Y":
            ...
            if kwargs["custom_tags"]:
                write_custom_rule(rule, **kwargs)
...
def main():
    ...
    if args.keyword == None:
        ...
        if args.tailor:
            ...
            if not args.keyword == tailored_filename:
                custom_tags = sanitised_input(f'Would you like to include ({tailored_filename}) as a custom tag in your rules? (y/N) ', str, default_="N").lower()
            ...
            if custom_tags == "y":
                custom_refs = sanitised_input(f'Would you like to include ({tailored_filename}) as a custom reference placeholder in your rules? (y/N) ', str, default_="N").lower()
                odv_baseline_rules = odv_query(found_rules, benchmark, custom_tags=tailored_filename, custom_refs=custom_refs)
            else:
