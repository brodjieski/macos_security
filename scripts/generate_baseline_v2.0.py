#!/usr/bin/env python3
# Title         : generate_baseline_v2.0.py
# Description   : Script to generate baseline files from the rule library
...
def main():
    # define the build path and ouput location
    _repo_path = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
    build_path = os.path.join(_repo_path, "build", "baselines")
    if not (os.path.isdir(build_path)):
        try:
            os.makedirs(build_path)
        except OSError:
            logging.ERROR(f"Creation of the directory {build_path} failed")

    # build the list of rules for the keyword provided
    baseline_rules = []
